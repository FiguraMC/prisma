 ...
 */
async function parse(overload, client) {
    ...
    for (let i = 0; i < overload.arguments.length; i++) {
        ...
        const validated = await argumentTypes.get(arg.type).validate(group, arg.options, client); // validate() throws CommandParseError
