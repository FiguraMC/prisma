...
module.exports.filter = async function (message) {
  const pinged = message.mentions.users.find(x => x.id == message.client.user.id)
    if (message.guild.id == process.env.MAIN_GUILD && (process.env.HELP_CHANNELS.split(',').find(x => x == message.channel.id || x == message.channel.parentId) || pinged)) {
        ...
        for (const [key, value] of DataStorage.storage.faq.entries()) {
            ...
            if (includesAll) {
                if (cooldowns.has(key) && cooldowns.get(key) > Date.now() && !pinged) continue; // still on cooldown
